...
if [[ -L /opt/fmc_repository/CommandDefinition/paloalto-ngfw-ms ]]; then
	SYMLINK_TARGET=$(readlink -f /opt/fmc_repository/CommandDefinition/paloalto-ngfw-ms || true)
	if [[ -n "$SYMLINK_TARGET" && -e "$SYMLINK_TARGET/.git" ]]; then
		log_info "🌹 Symlink and git repo found."
	else
		log_info "🦥 Removing symlink."
		rm -f /opt/fmc_repository/CommandDefinition/paloalto-ngfw-ms
	fi
fi

# for backward compatibility with old backend, move existing repository to CommandDefinition
if [[ -e /opt/fmc_repository/paloalto-ngfw-ms/.git ]]; then
	...
	if [[ -e /opt/fmc_repository/CommandDefinition/paloalto-ngfw-ms ]]; then
		log_info "🍄 Removing existing destination directory before move."
		rm -rf /opt/fmc_repository/CommandDefinition/paloalto-ngfw-ms
	fi
